    ...
    for (;;)
    {
        ...
        case 2: // register业务
        {
            ...
            else
            {
                ...
                if (-1 == len)
                {
                    cerr << name << "recv reg response error" << endl;
                }
                else
                {
                    json responsejs = json::parse(buffer);
...
void showCurrentUserDate()
{
    ...
    cout << "DEBUG: friend list size = " << g_currentUserFriendList.size() << endl;
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRuning)
    {
        ...
        // 调用显影命令的事件处理回调，mainMenu对修改封闭，添加新功能不需要修改该函数
